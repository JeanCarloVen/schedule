...
Function availability($typeTime, $start_wd, $end_wd, $start, $end){
    ...
    $end_booked = $end_booked->modify('+ 15 minute');
    
    //Intervalo de cuartos para el horario
    if($typeTime == 'isQuarterHour'){
        //Definición del intervalo para los horarios
        $interval = DateInterval::createFromDateString('15minute');
...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    if( isset($_GET['day']) && isset($_GET['id_supplier']) && isset($_GET['id_service']))
    {
        ...
        //Información por Staff
        //$staff = prepareUniquesSchedules($arr);

        //Formación del arreglo 
        $res[] = array_merge($general, $staff);
